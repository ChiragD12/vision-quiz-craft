import{i as e}from"./store-CWmZQzfB.js";var t=`https://generativelanguage.googleapis.com/v1beta/models`;function n(){let t=e.getApiKey();if(!t)throw Error(`Add your Gemini API key in Settings first.`);return t}function r(e){let t=e.match(/^data:([^;]+);base64,(.+)$/);if(!t)throw Error(`Unsupported file format.`);return{mimeType:t[1],data:t[2]}}async function i(r){let i=n(),a=e.getModel(),o={contents:[{role:`user`,parts:r.parts}],generationConfig:{temperature:r.temperature??.5,...r.responseSchema?{responseMimeType:`application/json`,responseSchema:r.responseSchema}:{}}};r.systemInstruction&&(o.systemInstruction={parts:[{text:r.systemInstruction}]});let s=await fetch(`${t}/${a}:generateContent?key=${encodeURIComponent(i)}`,{method:`POST`,headers:{"content-type":`application/json`},body:JSON.stringify(o)});if(!s.ok){let e=await s.text(),t=e;try{t=JSON.parse(e)?.error?.message||e}catch{}throw Error(`Gemini: ${t}`)}let c=(await s.json())?.candidates?.[0]?.content?.parts?.map(e=>e.text||``).join(``)??``;if(r.responseSchema)try{return JSON.parse(c)}catch{throw Error(`Gemini returned invalid JSON.`)}return c}async function a(e){let t=[];if(e.text?.trim()&&t.push(e.text.trim()),e.images?.length){let n=await i({parts:[{text:`Extract all readable text from these handwritten or printed UPSC study notes. Preserve structure, headings, and bullet points. Reply with only the extracted text, no commentary.`},...e.images.map(e=>({inlineData:r(e)}))],temperature:.1});n.trim()&&t.push(n.trim())}if(e.pdf){let n=await i({parts:[{text:`Extract all readable text from this PDF of UPSC study notes. Preserve structure. Reply with only the extracted text.`},{inlineData:r(e.pdf)}],temperature:.1});n.trim()&&t.push(n.trim())}return t.join(`

---

`)}var o={type:`OBJECT`,properties:{questions:{type:`ARRAY`,items:{type:`OBJECT`,properties:{question:{type:`STRING`},options:{type:`ARRAY`,items:{type:`STRING`}},answerIndex:{type:`INTEGER`},explanation:{type:`STRING`},difficulty:{type:`STRING`,enum:[`Easy`,`Medium`,`Hard`]}},required:[`question`,`options`,`answerIndex`,`explanation`,`difficulty`]}}},required:[`questions`]};function s(e){if(!(/match\s+the\s+following/i.test(e)||/(List I|Column I)/i.test(e)&&/(List II|Column II)/i.test(e)))return e;let t=e.replace(/\r\n/g,`
`);return t=t.replace(/(?<!\n)(List I(?:\s*\([^)]+\))?)/gi,`
...
$1`),t=t.replace(/(?<!\n)([A-E])\.\s*/g,`
$1. `),t=t.replace(/(?<!\n)([1-9])\.\s*/g,`
$1. `),t=t.replace(/\s*(Select the correct answer)/i,`

$1`),t=t.replace(/\n{3,}/g,`

`).replace(/[ \t]+\n/g,`
`).trim(),t}async function c(e){let t=`You are a senior UPSC Civil Services Prelims (General Studies Paper I) and HPSC Prelims question setter with years of experience drafting the actual exam. Generate exactly ${e.count} MCQs that would be indistinguishable from questions in a real UPSC CSE Prelims or HPSC Prelims paper — conceptual, elimination-based, and precisely worded, never simple one-line trivia.
...
- This randomization rule applies to every question type, including Match the Following, Consider the Following Statements, Consider the Following Pairs, Assertion–Reason, Chronology, and Direct Conceptual questions.`,n=((await i({parts:[{text:`Topic: ${e.topicName??`General`}

Notes:
${e.notes}

Return exactly ${e.count} questions.`}],systemInstruction:t,responseSchema:o,temperature:.7})).questions??[]).map(e=>({...e,question:s(e.question)})).filter(e=>e&&Array.isArray(e.options)&&e.options.length===4&&typeof e.answerIndex==`number`);if(n.length===0)throw Error(`Gemini returned no questions.`);return n.slice(0,e.count)}async function l(e){let t=[...e.existing];for(let e=t.length-1;e>0;e--){let n=Math.floor(Math.random()*(e+1));[t[e],t[n]]=[t[n],t[e]]}return t.slice(0,e.count)}export{l as n,c as r,a as t};