...
const MODELS = [
  { id: "gemini-3.6-flash", label: "Gemini 3.6 Flash", desc: "Fast, recommended" },
  { id: "gemini-3.5-flash", label: "Gemini 3.5 Flash", desc: "High capability" },
  { id: "gemini-3.5-flash-lite", label: "Gemini 3.5 Flash-Lite", desc: "Fast & economical" },
  { id: "gemini-3.1-flash-lite", label: "Gemini 3.1 Flash-Lite", desc: "Lowest-cost option" },
];
...
function SettingsPage() {
  ...
  // TEMP: "Repair Notifications" button/state — a manual escape hatch for
  // the iOS VapidPkHashMismatch issue (stale push subscription tied to an
  // old VAPID key). The automatic fix already lives in push-client.ts
  // (stale-key detection + rotation) and __root.tsx (runs on mount, and
  // again on 'pageshow'/tab-visible so it also catches resumed, not just
  // freshly-loaded, installed PWAs). This button exists for the case where
  // a device hasn't resumed/relaunched yet, or as a visible way to confirm
  // notifications are healthy without waiting on that. Remove this row
  // once the automatic path has been confirmed stable across devices.
  ...
  const repairNotifications = async () => {
    ...
    try {
      ...
      if (Notification.permission === "denied") {
        toast.error(
          "Notifications are blocked for this app. Enable them in your device's notification settings, then try again.",
        );
        return;
      }

      // ensurePushSubscription() is the existing helper — it already
      // contains the unsubscribe → resubscribe → POST /api/subscribe
      // rotation logic for a stale VAPID key, and only prompts for
      // permission if it isn't already granted. Nothing here duplicates
      // that logic or touches localStorage/IndexedDB/app data.
      ...
      if (subscription) {
        toast.success("Notifications allowed — you're all set.");
      } else {
        toast.error("Couldn't set up notifications. Please try again.");
      }
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Notification permission failed.");
    } finally {
    ...
    <div className="min-h-screen relative overflow-x-hidden">
      ...
      <main className="relative z-10 mx-auto max-w-2xl px-5 py-6 space-y-5">
        ...
        <Group delay={0.06}>
          ...
          {/* TEMP: remove this row once the automatic push-subscription
              rotation (push-client.ts + __root.tsx resume check) has been
              confirmed stable across devices — see comment on
              repairNotifications above. */}
          <Row
            icon={Bell}
            title="Notifications"
            open={openRow === "notifications"}
            onToggle={() => toggleRow("notifications")}
          >
            <div className="pt-2 space-y-2">
              <p className="text-sm text-muted-foreground">
                If push notifications stop arriving on this device, tap this
                to re-register with the app's current key. It only touches
                the browser's push subscription — your notes, quizzes, and
                progress are never affected.
              </p>
              <motion.div
              ...
              >
                ...
                >
                  ...
                  {repairingPush ? "…" : "Allow Notifications"}
                </Button>
              </motion.div>
            </div>
          </Row>
