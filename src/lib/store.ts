...
// Obsolete Gemini model IDs that must be migrated to the current default
// whenever they're found in an existing user's stored DB.
...
export function load(): DB {
  ...
  let changed = seedCore(db);
  if (db.gemini_model && OBSOLETE_GEMINI_MODELS.has(db.gemini_model)) {
    db.gemini_model = "gemini-3.6-flash";
    changed = true;
  }
...
export const api = {
  ...
  getModel(): string {
    return load().gemini_model || "gemini-3.6-flash";
  },
  setModel(m: string) {
    update((db) => {
      db.gemini_model = m;
